...
[\log p(X, Z)] + \const,
\]
in which the expectation is with respect to $q$.
...
\[
\log p(X) = \L - \sum_m \sum_Z q(Z \mid m) q(m) 
\log \frac{p(Z, m \mid X)}{q(Z \mid m) q(m)},
\]
where $\L$ is a lower bound for $\log p(X)$ given 
by $\L = \sum_m \L_m$ and
\[
\L_m = \sum_Z q(Z \mid m) q(m) 
\log \frac{p(X, Z, m)}{q(Z \mid m) q(m)}.
\]

Now, for each model $m$, $\L_m$ is in fact 
a lower bound for $\log p(X \mid m)$. Meanwhile, we have
\[
p(m \mid X) \propto p(X \mid m) p(m).
\]
This is saying that if we assign equal prior $p(m)$ for each model $m$,
we can interprete $\L_m$ as an approximation to the posterior
$p(m \mid X)$, and we can do model selection using the value 
of $\L_m$ for each model $m$. This should be contrasted with a
...
Here we have a useful inequality. For any vector $x$ and square matrix 
$A$, we have 
\[
x^T A x = \tr (x x^T A) = \tr (A x x^T). 
\]
Indeed, using Einstein summation, we have $x^T A x = x_i A_{ij} x_j$ and 
$\tr (A x x^T) = A_{ij} x_j x_i$. 
